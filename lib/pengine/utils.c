...
node_list_exclude(GListPtr list1, GListPtr list2, gboolean merge_scores)
{
    ...
    /* パラメータ左リストのノードが、パラメータ右リストに含まれるか検索する */
	
	other_node = pe_find_node_id(list2, node->details->id);
	
	if(other_node == NULL) {
		/* 右リストに含まれない場合、左リストのノード情報のweightに-INFINITYをセットする */
	    node->weight = -INFINITY;
	} else if(merge_scores) {
		/* merge_scoresパラメータがtrueの場合は、スコアをマージして、左リストのノード情報にセットする */
	...
	/* 処理されたパラメータ左リストから、パラメータ右リストのノードを検索する */
	other_node = pe_find_node_id(result, node->details->id);
	
	if(other_node == NULL) {
		/* 処理された左リストに右リストのノード情報が存在しない場合 */
	    node_t *new_node = node_copy(node);
	    new_node->weight = -INFINITY;
	    /* 存在しないノード情報を、処理された左リストに追加して、weigthを-INFINITYにセットする */
...
/* opノードを展開する
          <op id="postgresql-monitor-10s" interval="10s" name="monitor" on-fail="restart" timeout="60s"/>
	...
	action_t *action, xmlNode *xml_obj, pe_working_set_t* data_set)
{
	...
	*/
	...
	/* opノードのmeta_attributesノードを展開してaction->metaハッシュテーブルにセットする */
	unpack_instance_attributes(data_set->input, xml_obj, XML_TAG_META_SETS,
				   NULL, action->meta, NULL, FALSE, data_set->now);

	/* opノードのinstance_attributesノードを展開してaction->metaハッシュテーブルにセットする */
	...
	/* action->metaハッシュテーブルから"on-fail"値を取り出す */
	...
	if(safe_str_eq(action->task, CRMD_ACTION_STOP)
	   && safe_str_eq(value, "standby")) {
		/* 対象アクションがstopの場合で、"on-fail=standby"の場合は、値にNULLをセットする */
	    crm_config_err("on-fail=standby is not allowed for stop actions: %s", action->rsc->id);
	    value = NULL;
	}

	if(value == NULL) {
		/* "on-fail"値が設定されていない場合（取り出せない場合)は以降で、デフォルトセットを行う */
	...
	if(value == NULL && safe_str_eq(action->task, CRMD_ACTION_STOP)) {
		/* opがcibに設定されていない場合で、アクションがstopの場合(opのon-failが設定されていない場合も含む) */
		if(is_set(data_set->flags, pe_flag_stonith_enabled)) {
			/* stonith-enabled=trueの場合は、on-fail=fence */
			action->on_fail = action_fail_fence;		
			value = "resource fence (default)";
			
		} else {
			/* falseの場合は、on-fail=block */
			action->on_fail = action_fail_block;		
