		  ...
		  int factor, gboolean allow_rollback, gboolean only_positive) 
{
    ...
    if(can_run_any(work)) {
		/* 組み合わせるリソースのノード配置候補情報で更新した結果、*/
		/* 対象リソースの実行可能なノード配置先がある場合 */
		
		
		/* 組み合わせるリソースがwith-rsc指定されているcolocation情報を全てたぐって */
		/* 対象リソースの複製情報を更新する */
		slist_iter(
...
native_color(resource_t *rsc, pe_working_set_t *data_set)
{
	...
	if(rsc->parent && is_not_set(rsc->parent->flags, pe_rsc_allocating)) {
		...
		/* 親リソースの配置処理が始まっていない場合は、親リソースのcolorを先に処理する */
		/* 配置処理は親リソースから始まる為 */
	...
	/* 処理前のallowed_nodesをダンプする */
	dump_node_scores(alloc_details, rsc, "Pre-allloc", rsc->allowed_nodes);

	/* このリソースがrsc指定されているcolocation情報(rsc_cons)を全て処理する */
	/* rsc指定されていない場合は、ループしないので注意 */
	slist_iter(
		...
		if(constraint->role_lh >= RSC_ROLE_MASTER
		   || (constraint->score < 0 && constraint->score > -INFINITY)) {
			/* colocationのrsc側のroleがMASTER以上か */
			/* colocationのスコアがマイナスで、-INFINITY以外の場合 */
			/* このリソースの配置候補のノード情報を複製しておく */
		    archive = node_list_dup(rsc->allowed_nodes, FALSE, FALSE);
		}
		
		/* with-rsc指定されているリソースのcolorを先に処理する */
		rsc_rh->cmds->color(rsc_rh, data_set);
		
		/* with-rscのcolorが決まった（もしくは決まっているので、) */
		/* このリソースのrsc指定されているcolocation情報(rsc_cons)を処理する */
		/* --- colocationのスコアを条件に合えば、rsc指定の配置候補ノードのweightに加算することになる --- */
		rsc->cmds->rsc_colocation_lh(rsc, rsc_rh, constraint);	
		
		if(archive && can_run_any(rsc->allowed_nodes) == FALSE) {
			/* 上記で複製済みで、リソースの配置候補ノードがない場合は、複製情報でこのリソース配置候補ノードを */
			/* 戻す */
	...
	/* colocationを適用した後のスコアをダンプする */
	dump_node_scores(alloc_details, rsc, "Post-coloc", rsc->allowed_nodes);

	/* このリソースがwith-rsc指定されているcolocation情報(rsc_cons_lhs)を全て処理する */
	...
	/* このリソースの次のROLEが停止の場合は、このリソースのスコアを-INFINITYで更新する */
...
/* primitvieリソースのcolocationのrsc指定を処理する */
	...
	resource_t *rsc_lh, resource_t *rsc_rh, rsc_colocation_t *constraint)
{
	...
	/* with-rsc指定側の処理で、colocationを処理する */
	...
	resource_t *rsc_lh, resource_t *rsc_rh, rsc_colocation_t *constraint)
{
	...
	if(constraint->score > 0
	   ...
	   && constraint->role_lh != rsc_lh->next_role) {
		...
		/* colocationのスコアが０以上で、*/
		/* colocationのrsc指定のロールがRSC_ROLE_UNKNOWN以外で、かつ、 */
		/* colocationのrsc指定のロールが次のrsc指定リソースのロールでない場合は処理しない */
		return FALSE;
	}
	...
	if(constraint->score > 0
	   ...
	   && constraint->role_rh != rsc_rh->next_role) {
		do_crm_log_unlikely(level, "RH: Skipping constraint: \"%s\" state filter",
			    role2text(constraint->role_rh));
		/* colocationのスコアが０以上で、*/
		/* colocationのwith-rsc指定のロールがRSC_ROLE_UNKNOWN以外で、かつ、 */
		/* colocationのwith-rsc指定のロールが次のwith-rsc指定リソースのロールでない場合は処理しない */
		return FALSE;
	}
	...
	if(constraint->score < 0
	   ...
	   && constraint->role_lh == rsc_lh->next_role) {
		...
		/* colocationのスコアが負数で、*/
		/* colocationのrsc指定のロールがRSC_ROLE_UNKNOWN以外で、かつ、 */
		/* colocationのrsc指定のロールが次のrsc指定リソースのロールと一致した場合は処理しない */
		return FALSE;
	}
	...
	if(constraint->score < 0
	   ...
	   && constraint->role_rh == rsc_rh->next_role) {
		...
		/* colocationのスコアが負数で、*/
		/* colocationのwith-rsc指定のロールがRSC_ROLE_UNKNOWN以外で、かつ、 */
		/* colocationのwith-rsc指定のロールが次のwith-rsc指定リソースのロールと一致した場合は処理しない */
		return FALSE;
	}
	...
	resource_t *rsc_lh, resource_t *rsc_rh, rsc_colocation_t *constraint) 
{
	...
	if(rsc_rh->allocated_to) {
		/* with-rsc指定の配置先(allocated_to)が決まっている場合は、その情報を取り出す */
		 ...
		 */
		 /* colocationのスコアが負数の場合は処理しない */
		return;
	}

	/* rsc指定リソースの配置候補ノード情報を複製する */
	work = node_list_dup(rsc_lh->allowed_nodes, FALSE, FALSE);
	
	/* 複製したrsc指定リソースの配置候補ノードを全て処理する */
	slist_iter(
		node, node_t, work, lpc,
		/* 複製したノード情報のid情報を取得する */
		tmp = g_hash_table_lookup(node->details->attrs, attribute);
		if(do_check && safe_str_eq(tmp, value)) {
			/* with-src指定のリソースの配置先が決まっていて、そのノードと同じノードのid情報がrsc指定側 */
			/* とマッチした場合(同じ配置先ノードが配置可能) */
		    if(constraint->score < INFINITY) {
				/* colocationのスコアが整数でINFINITY以外の場合は、rscリソースのノードに加算する */
			...
			/* with-src指定のリソースの配置先が決まっていないか、そのノードと同じノードのid情報がrsc指定側 */
			/* とマッチしない場合で */
			/* colocationのスコアがINFINITYの場合 */
			crm_debug_2("%s: %s.%s -= %d (%s)", constraint->id, rsc_lh->id,
				    node->details->uname, constraint->score, do_check?"failed":"unallocated");
			/* rsc指定のそのノードに-INFINITYを加算した値を設定して、配置できないノードにする */
			...
			/* 
			   ...
			   colocationのwith-rsc指定側の配置が決まっていない間
			    
			      または、
			      
			      
			   colocationのwith-rscの配置先と同じノードでないノードでは
			   
			   
			   rsc指定側のリソースは起動できなくなる....
	...
	if(can_run_any(work)
	   ...
	   || constraint->score >= INFINITY) {
		...
		/* 上記のwith-rsc指定との処理後のノード情報で
		
		  rsc指定のリソースがどこかで起動可能か？
		  colocationのスコアがINIFINITYか-INFINITYの場合は、
		  
		  そのまま、上記で処理された配置候補ノード情報を、rsc指定リソース側の配置候補ノード情報にセットする
		...
		*/
		...
		/* 上記以外(スコアが数値もしくは、rsc側の起動可能なノードがない場合 */
		/* ログを出す */
...
/* primitvieリソースのcolocationのwith-rsc指定を処理する */
	...
	resource_t *rsc_lh, resource_t *rsc_rh, rsc_colocation_t *constraint)
{
	...
	if(is_set(rsc_rh->flags, pe_rsc_provisional)) {
		...
		if((constraint->score > -INFINITY) && (constraint->score < INFINITY)) {
			/* colocationのスコアが数値スコア(INFINITY,-INFINITY以外)なら処理しない */
			return;
		}

		/* rscリソースと、with-rscリソースの配置ノード情報を取り出す */
		...
		if(constraint->score == INFINITY && details_lh != details_rh) {
			/* colocationのスコアがINFINITYで、rscリソースとwith-rscリソースの配置先が違う場合は */
			/* colocationの制約に違反している配置となっているのでエラーログを出力する */
			...
			/* colocationのスコアが-INFINITYで、rscリソースとwith-rscリソースの配置先が同じ場合は */
			/* colocationの制約に違反している配置となっているのでエラーログを出力する */
			crm_err("%s and %s are both allocated"
