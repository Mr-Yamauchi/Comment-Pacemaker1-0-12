...
group_color(resource_t *rsc, pe_working_set_t *data_set)
{
	...
	if(group_data->first_child == NULL) {
	    /* nothign to allocate */
		/* 子リソースが存在しないgroupリソースの場合は、配置先決定済みとして処理を終了 */
	...
	/* groupリソースの先頭の子リソースのcolocation情報(自身がrsc指定されている情報:rsc_cons)に */
	/* groupリソースのcolocation情報(自身がrsc指定されている情報:rsc_cons)を追加する */
	group_data->first_child->rsc_cons = g_list_concat(
		group_data->first_child->rsc_cons, rsc->rsc_cons);
    /* groupリソース自体のcolocation情報(自身がrsc指定されている情報:rsc_cons)はクリアする */
	rsc->rsc_cons = NULL;

	/* groupリソースの先頭の子リソースのcolocation情報(自身がwith-rsc指定されている情報:rsc_cons_lhs)に */
	/* groupリソースのcolocation情報(自身がwith-rsc指定されている情報:rsc_cons_lhs)を追加する */
	group_data->first_child->rsc_cons_lhs = g_list_concat(
		group_data->first_child->rsc_cons_lhs, rsc->rsc_cons_lhs);
    /* groupリソース自体のcolocation情報(自身がwith-rsc指定されている情報:rsc_cons_lhs)はクリアする */
	...
	/* groupリソースの次ロールを先頭の子リソースの次のロールでセットする */
